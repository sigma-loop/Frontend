...
- **Math Editor**: MathLive (`mathlive`) `<math-field>` — WYSIWYG visual editor + on-screen math keyboard for the student's MATH answer (emits LaTeX). Fonts are served from `public/mathlive/fonts` (`MathfieldElement.fontsDirectory`)
- **LaTeX**: KaTeX (via `rehype-katex`, `remark-math`) — renders MATH problem statements and the grader's verdict/rationale
...
│   │       ├── MathWorkspace.tsx         # MathLive visual editor (MathEditor) + verdict panel
...
- A MathLive `<math-field>` WYSIWYG editor (`MathEditor`) with an on-screen math keyboard, so students build the answer visually instead of typing raw LaTeX. The field still emits LaTeX (kept in `latex` state / localStorage and submitted unchanged); a collapsible "LaTeX Source" panel shows exactly what will be submitted.
...
- **Design language: "clean & technical"** (Linear/Vercel) — flat, hairline-bordered
  surfaces; one restrained indigo accent; **no gradient text/tiles/buttons, no
  decorative blur blobs, no colored glow shadows.** See `DESIGN_SYSTEM.md`.
- Custom CSS utilities in `index.css`: `.glass-panel` / `.glass-card` (now **flat**
  `bg-white dark:bg-[#161b22]` + hairline border — the names are historical, not
  glass), `.text-gradient` (now a **solid** indigo accent), `.icon-tile` (flat accent
  icon tile, replaces gradient tiles), `.eyebrow` (mono uppercase kicker label).
- Radii: cards/panels `rounded-xl`, buttons/inputs `rounded-lg`, badges `rounded-md`;
  avatars/dots/progress stay `rounded-full`. Buttons are not pill-shaped.
- Fonts: Inter (body), Outfit (display/headings), system `font-mono` for technical labels.
- Color palette: Indigo primary + violet accent, gray neutrals; semantic green/amber/red/blue.
...
| `pages/Curriculum/Curriculum.tsx` (hardcoded)  | Removed — the mentor _generates_ the curriculum |
